...
class BaselineModel(torch.nn.Module):
    ...
    def feat2emb(self, seq, feature_tensors, mask=None, include_user=False):
        ...
        if include_user:
            ...
            # print(f"shape user_emb{user_embedding.shape} \n user_mask {user_mask[0]}, sstr :{sstr}")
        ...
        # 直接使用预处理好的tensor进行embedding，并融入时间上下文
        # ii = 0
        for feat_dict, feat_type, feat_list in all_feat_types:
            ...
            for k in feat_dict:
                ...
                tensor_feature = feature_tensors[k]
                # print(f"shape of tensor : {tensor_feature.shape}")

                # 这里不需要mask因为需要保证特征形状
                if feat_type.endswith('sparse'):
                    # ii += 1
                    # if k == '300':
                    #     print(f"k == 300 {tensor_feature[0]}")
                    tensor_feature = tensor_feature.to(self.dev)
                    feat_emb = self.sparse_emb[k](tensor_feature)
                    feat_list.append(feat_emb)
                elif feat_type.endswith('array'):
                    tensor_feature = tensor_feature.to(self.dev)
                    feat_emb = self.sparse_emb[k](tensor_feature).sum(2)
                    feat_list.append(feat_emb)
                elif feat_type.endswith('continual'):
                    batch_size, seq_len, feat_dim = tensor_feature.shape
                    chunk_size = 8
                    try:
                        dummy = torch.zeros(1, chunk_size, 1, device=self.dev, dtype=torch.float32)
                        with torch.no_grad():
                            # 模拟三步变换流程进行CUDA预热
                            dummy_linear = self.continual_transform['304'][0](dummy)
                            dummy_norm = self.continual_transform['304'][1](dummy_linear)
                            dummy_final = self.continual_transform['304'][2](dummy_norm)
                            # 立即清理dummy张量
                            del dummy, dummy_linear, dummy_norm, dummy_final
                        torch.cuda.empty_cache()
                        # print(f"    ✅ CUDA warm-up completed successfully")
                    except Exception as e:
                        print(f"    ⚠️ CUDA warm-up failed (continuing anyway): {e}")
                        torch.cuda.empty_cache()
                    ...
                    for start_idx in range(0, seq_len, chunk_size):
                        ...
                        # 只转移当前需要的小块到GPU
                        chunk_cpu = tensor_feature[:, start_idx:end_idx, :]
                        ...
                        try:
                            ...
                            with torch.no_grad():
                                # 第一步：Linear变换
                                linear_out = self.continual_transform['304'][0](chunk_gpu)
                                del chunk_gpu  # 立即释放输入
                                
                                # 第二步：RMSNorm
                                norm_out = self.continual_transform['304'][1](linear_out)
                                del linear_out  # 立即释放中间结果
                                
                                # 第三步：SwiGLU激活
                                final_out = self.continual_transform['304'][2](norm_out)
                                del norm_out  # 立即释放中间结果
                                
                            transformed_chunks.append(final_out)
                            
                        except RuntimeError as e:
                            print(f"    ❌ 304 feature chunk {start_idx}-{end_idx} failed: {e}")
                            # 失败时创建零张量
                            zero_chunk = torch.zeros(
                                batch_size, end_idx - start_idx, self.continual_transform['304'][0].out_features,
                    ...
                    if transformed_chunks:
                        ...
                        feat_list.append(transformed_emb)
                        del transformed_chunks, transformed_emb
                    # feat_emb = self.continual_transform[k](tensor_feature)
                    # feat_list.append(feat_emb)
