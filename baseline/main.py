...
if __name__ == '__main__':
    ...
    for epoch in range(epoch_start_idx, args.num_epochs + 1):
        ...
        for step, batch in tqdm(enumerate(train_loader), total=len(train_loader)):
            seq, pos, neg, token_type, next_token_type, next_action_type, seq_feat, pos_feat, neg_feat, aug_neg_feat1, aug_neg_feat2 = batch
            ...
            # 使用增强的负样本特征进行自监督学习
            ssl_loss = model.ssl_forward(
                seq, pos, neg, token_type, next_token_type, next_action_type, seq_feat, pos_feat, neg_feat, 
                aug_neg_feat1, aug_neg_feat2, global_step
            )
            ...
            log_json = json.dumps(
                {'global_step': global_step, 'loss': loss.item(), 'epoch': epoch, 'time': time.time(),
                 'ssl_loss': ssl_loss.item(), 'bce_loss': bce_loss.item()}
            )
